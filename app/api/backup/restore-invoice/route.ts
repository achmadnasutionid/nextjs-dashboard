...
/**
 * POST /api/backup/restore-invoice – create an invoice from backup JSON (e.g. from Drive).
 * Body: backup JSON (invoiceId, companyName, items, remarks, signatures, ...).
 * Fails with 409 if an invoice with that invoiceId already exists.
 */
export async function POST(request: Request) {
  try {
    ...
    if (!invoiceId) {
      return NextResponse.json({ error: "Missing invoiceId in backup data" }, { status: 400 })
    }
    ...
    if (existing) {
      return NextResponse.json(
        { error: `An invoice with ID ${invoiceId} already exists. Cannot restore to avoid overwriting.` },
        { status: 409 }
      )
    }
