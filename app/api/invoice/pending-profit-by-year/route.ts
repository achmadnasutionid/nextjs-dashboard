...
/**
 * GET pending profit by year: for each pending invoice (Invoice + Paragon + Erha),
 * look up tracker by invoiceId and sum tracker "photographer" (totalAmount - expense) by year.
 * Returns { years, byYear, missingIds }.
 * missingIds = document IDs that are pending but have no matching tracker (for redirect card).
 */
export async function GET() {
  try {
    ...
    console.error("Error fetching pending profit by year:", error)
    return NextResponse.json(
      { error: "Failed to fetch pending profit" },
      { status: 500 }
    )
  }
}
