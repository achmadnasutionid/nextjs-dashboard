...
export async function GET(
  ...
  try {
    ...
    console.error("Error fetching barclay ticket:", error)
    return NextResponse.json(
      { error: "Failed to fetch barclay ticket" },
      { status: 500 }
    )
  }
}

// PUT update barclay ticket
export async function PUT(
  ...
  try {
    ...
    const current = await prisma.barclayTicket.findUnique({
      where: { id },
      select: { status: true }
    })
    ...
    if (body.status && Object.keys(body).length === 1) {
      ...
      const ticket = await prisma.barclayTicket.update({
        where: { id },
        data: {
          status: newStatus
        },
    ...
    const uniqueBillTo = combined ? await generateUniqueName(combined, 'barclay', id) : combined

    const originalTicket = await prisma.barclayTicket.findUnique({
      where: { id },
    ...
    const ticket = await prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.barclayTicket.update({
        where: { id },
        data: {
      ...
      const existingItems = await tx.barclayTicketItem.findMany({
        where: { ticketId: id },
        select: { id: true }
      })
      ...
      const deleteDetailsPromise = itemIdsToDeleteDetails.length > 0
        ? tx.barclayTicketItemDetail.deleteMany({
      ...
      if (allNewDetails.length > 0) {
        await tx.barclayTicketItemDetail.createMany({
          data: allNewDetails
        })
      }
      ...
      await tx.barclayTicketItem.deleteMany({
        where: {
          ticketId: id,
      ...
      const existingRemarks = await tx.barclayTicketRemark.findMany({
        where: { ticketId: id },
        select: { id: true }
      })
      ...
      if (remarksToCreate.length > 0) {
        ...
        await tx.barclayTicketRemark.createMany({
          data: remarkData
        })
        
        // Fetch newly created remarks
        const newRemarks = await tx.barclayTicketRemark.findMany({
          where: {
            ticketId: id,
      ...
      await tx.barclayTicketRemark.deleteMany({
        where: {
          ticketId: id,
          id: { notIn: idsToKeep }
        }
      ...
      return tx.barclayTicket.findUnique({
        where: { id },
        include: {
          items: {
            include: {
    ...
    // Invalidate caches after updating barclay ticket
    await invalidateBarclayCaches(id)

    return NextResponse.json(ticket)
  } catch (error) {
    console.error("Error updating barclay ticket:", error)
    ...
    const userMessage = isPayloadTooLarge
      ...
      : message || "Failed to update barclay ticket"
    return NextResponse.json(
      { error: userMessage },
      { status: 500 }
    )
